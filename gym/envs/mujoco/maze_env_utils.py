...
def plot_ray(self, reading, ray_idx, color='r'):
    structure = self.unwrapped.MAZE_STRUCTURE
    size_scaling = self.unwrapped.MAZE_SIZE_SCALING
    ...
    robot_xy = np.array(self.unwrapped.get_body_com("torso")[:2])  # the coordinates of this are wrt the init!!
    ori = self.unwrapped.get_ori()  # for Ant this is computed with atan2, which gives [-pi, pi]

    # compute origin cell i_o, j_o coordinates and center of it x_o, y_o (with 0,0 in the top-right corner of struc)
    o_xy = np.array(self.unwrapped._find_robot())  # this is self.unwrapped.init_torso_x, self.unwrapped.init_torso_y !!: center of the cell xy!
    ...
    # for ray_idx in range(self.unwrapped._n_bins):
    length_wall = self.unwrapped._sensor_range - reading * self.unwrapped._sensor_range if reading else 1e-6
    ray_ori = ori - self.unwrapped._sensor_span * 0.5 + ray_idx / (self.unwrapped._n_bins - 1) * self.unwrapped._sensor_span
    if ray_ori > math.pi:
        ray_ori -= 2 * math.pi
...
def plot_state(self, name='sensors'):
    structure = self.unwrapped.MAZE_STRUCTURE
    size_scaling = self.unwrapped.MAZE_SIZE_SCALING
    # duplicate cells to plot the maze
    structure_plot = np.zeros(((len(structure) - 1) * 2, (len(structure[0]) - 1) * 2))
    ...
    obs = self.unwrapped._get_maze_obs()
    ...
    for ray_idx in range(self.unwrapped._n_bins):
        length_wall = self.unwrapped._sensor_range - obs[ray_idx] * self.unwrapped._sensor_range if obs[ray_idx] else 1e-6
        ray_ori = ori - self.unwrapped._sensor_span * 0.5 + ray_idx / (self.unwrapped._n_bins - 1) * self.unwrapped._sensor_span
        if ray_ori > math.pi:
            ray_ori -= 2 * math.pi
        elif ray_ori < - math.pi:
        ...
        length_goal = self.unwrapped._sensor_range - obs[ray_idx + self.unwrapped._n_bins] * self.unwrapped._sensor_range if obs[
            ray_idx + self.unwrapped._n_bins] else 1e-6
        ray_ori = ori - self.unwrapped._sensor_span * 0.5 + ray_idx / (self.unwrapped._n_bins - 1) * self.unwrapped._sensor_span
